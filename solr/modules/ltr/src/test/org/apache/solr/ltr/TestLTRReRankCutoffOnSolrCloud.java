...
public class TestLTRReRankCutoffOnSolrCloud extends AbstractLTRSolrCloudTestBase {
  ...
  @Test
  public void distributedRerankCutoffScore_shouldBeReturnedPerShard() throws Exception {
    final SolrQuery query = newBaseRerankQuery();
    query.add("rq", "{!ltr model=powpularityS-model reRankDocs=3 echoReRankCutoff=true}");

    final QueryResponse queryResponse = solrCluster.getSolrClient().query(COLLECTION, query);
    ...
    assertEquals(NUM_SHARDS, perShardCutoff.size());
