...
public class ReRankCollector extends TopDocsCollector<ScoreDoc> {
  ...
  public TopDocs topDocs(int start, int howMany) {
    ...
    try {
      ...
      final Object reRankCutoffScore = getReRankCutoffScore(mainScoreDocs);
      ...
      updateResponseHeader(reRankCutoffScore);
  ...
  private Object getReRankCutoffScore(ScoreDoc[] mainScoreDocs) {
  ...
  private void updateResponseHeader(Object reRankCutoffScore) {
    SolrRequestInfo info = SolrRequestInfo.getRequestInfo();
    ...
    if (existingIndex >= 0) {
      responseHeader.setVal(existingIndex, reRankCutoffScore);
    } else {
      responseHeader.add(AbstractReRankQuery.RERANK_CUTOFF_RESPONSE_HEADER_KEY, reRankCutoffScore);
    }
  }
