...
When set to `true`, Solr adds a `reRankCutoff` to the responseHeader. For the default first-pass relevance ranking, the value is the relevance score of the last document eligible for reranking. When an explicit sort is used, `reRankCutoff` reflects the sort value of that last eligible document. If multiple sort criteria are used, Solr returns the cutoff values in sort order.
...
For an explicit multi-sort such as `sort=description desc,id asc`, the response header contains each cutoff value in order:
...
For distributed queries, Solr returns shard-local cutoff values in `reRankCutoffByShard`:
...
For non-distributed queries, Solr returns `reRankCutoff` instead of `reRankCutoffByShard`.
